...
# get data location
dirs <- list.dirs(path = 'datasets/integration/', recursive = F, full.names = F)

for(x in dirs){
  name <- gsub('_filtered_feature_bc_matrix','', x)
  
  cts <- ReadMtx(mtx = paste0('datasets/integration/',x,'/matrix.mtx.gz'),
                 features = paste0('datasets/integration/',x,'/features.tsv.gz'),
                 cells = paste0('datasets/integration/',x,'/barcodes.tsv.gz'))
...
# takes very long to run on non-Pro laptops...
